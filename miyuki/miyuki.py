    ...
    while retries < inner_retry:
        try:
            response = requests.get(
                url=request_url, headers=headers, timeout=inner_timeout, verify=False
            ).content
            return response
...
def thread_task(
    start, end, uuid, resolution, movie_name, video_offset_max, retry, delay, timeout
):
...
def video_download_jpegs(
    intervals, uuid, resolution, movie_name, video_offset_max, retry, delay, timeout
):
    thread_task_list = []
...
def download(
    ...
    if cover_action:
        try:
            ...
            with open(
                root + movie_save_path_root + "/" + movie_name + "-cover.jpg", "wb"
            ) as file:
...
def execute_download(args):
    ...
    if root is None:
        ...
        movie_save_path_root = root + movie_save_path_root
...
def main():
    ...
    parser.add_argument(
        "-ffmpeg", action="store_true", required=False, help="Enable ffmpeg processing"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "-noban", action="store_true", required=False, help="Do not display the banner"
    )
    parser.add_argument(
